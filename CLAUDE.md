...
- **Database: PostgreSQL (Npgsql).** This is the target (PRD + architecture doc). The scaffolded code still uses **SQL Server** everywhere (Aspire container, EF provider, migrations) — that's leftover template wiring to be migrated to Npgsql, not a design choice. New persistence work should target PostgreSQL.
- **Mediation: the source-generated `Mediator` library** (martinothamar), not MediatR. Both the docs and the code now agree on this — see the Mediator section below.
...
dotnet run --project src/Parkin.AspireHost      # full local stack (recommended): SQL Server + Papercut SMTP + API
dotnet run --project src/Parkin.Web             # API only, against (localdb)\mssqllocaldb (ConnectionStrings:AppDb in appsettings.json)
```

- **Aspire** (`src/Parkin.AspireHost/AppHost.cs`) provisions a persistent SQL Server container (`AppDb`) and a Papercut SMTP container, then launches the Web project. The Web project hard-requires a connection string named **`AppDb`** (guarded in `InfrastructureServiceExtensions`).
...
```powershell
dotnet ef migrations add <Name> --project src/Parkin.Web
dotnet ef database update    --project src/Parkin.Web
```

`AppDbContextFactory` provides the design-time context. Migrations live in `src/Parkin.Web/Infrastructure/Data/Migrations/`.
...
`src/Parkin.Web/config.nsdepcop` is compiled with `NSDEPCOP01` as an **error** — violations fail the build. Current rules forbid:
