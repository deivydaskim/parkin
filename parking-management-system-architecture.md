...
| Spaces | `GET/POST /api/v1/lots/{lotId}/spaces`, `PATCH …/{id}`, `POST …/{id}/deactivate` | **Operator+** | B4–B5; POST/PATCH accept optional `zone` (empty string clears) + `placement`; PATCH `clearPlacement: true` un-places. `PATCH /lots/{id}` accepts `layout` / `clearLayout: true` |
| Lot layout | `GET /api/v1/lots/{lotId}/layout`, `PUT /api/v1/lots/{lotId}/layout` | **Operator+** | B5, F1–F2. GET = read projection (lot footprint + every space with placement + active-reservation assignee). PUT = atomic batch `{ layout?, clearLayout?, spaces: [{ spaceId, placement|null, zone? }] }` (omitted layout = keep; omitted zone = keep), returns the updated projection, one `lot.layout_applied` audit entry\|null, zone? }] }`, one audit entry |
