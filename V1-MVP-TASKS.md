...
- [ ] **T4.2 — D2 Reassign.** *Deps: T4.1.* `POST /reservations/{id}/reassign` — atomically ends the prior reservation and creates a new one in a single request (never two ACTIVE rows, no window where the space is briefly unreserved), as an alternative to the FE's current cancel-then-create round trip; audit-logged with actor.
