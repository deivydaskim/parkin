...
- [x] **T1.4 — A5 API key management.** *Deps: T1.2.* Generate key (**shown once**, SHA-256 hashed at rest, store prefix for display), list, revoke — `SystemAdmin` only; ≥1 active key required for later ingestion; create/revoke audit-logged. Add `X-Api-Key` `AuthenticationHandler<ApiKeyAuthenticationOptions>` in `Infrastructure/ApiKeys`. FE: view-once generate dialog under `settings/`.
...
- [x] **T4.1 — D1 Create reservation.** *Deps: T2.2, T3.1.* `Reservation` aggregate (denormalized `lot_id`; no start/end date — a reservation is simply ACTIVE or CANCELLED, no window). `POST /reservations`, `GET /spaces/{spaceId}/reservation` (returns the space's single ACTIVE reservation, or null — no list endpoint since nothing needs one), `POST /reservations/{id}/cancel` (`Operator+`). **At most one ACTIVE reservation per space** → `409` on conflict (`ux_reservation_active_space`); ≤1 active per driver per lot (`ux_reservation_active_driver_lot`); creating sets space type RESERVED + removes from general pool; reserved driver implicitly has lot access even if RESTRICTED. No `created_by`/`created_at` on the entity — creation/cancellation are captured via the audit log, not on the aggregate. FE: `features/reservations/` (`ReservationDialog` — opened from the "Reservation" button on a Reserved-type space's row in the spaces table, fetching the space's active reservation only while the dialog is open; a single driver `Select` with a "— None —" option assigns or frees the space in place, no separate page).
  - **AC:** second active reservation on a space returns 409; space flips to RESERVED; cancelling (picking "— None —") frees the space for a new assignment; **integration test proves the partial unique index rejects the conflict under concurrency.**
  - *No Docker in this environment, so `dotnet ef database update` / Testcontainers-backed integration test / live Aspire+browser smoke test were not run — migrations generated (`ux_reservation_active_space` + `ux_reservation_active_driver_lot` partial unique indexes confirmed in the generated file), `dotnet build` (0 warnings/errors) + unit tests (`ReservationTests`, `CreateReservationHandlerTests`, 28/28 passing) + frontend `pnpm build`/`pnpm lint` were verified. `ReservationConcurrencyTests` (Testcontainers) is written but unexecuted locally.*
- [x] **T4.2 — D2 Reassign.** *Deps: T4.1.* `POST /reservations/{id}/reassign` — atomically ends the prior reservation and creates a new one in a single request (never two ACTIVE rows, no window where the space is briefly unreserved), as an alternative to the FE's current cancel-then-create round trip; audit-logged with actor.
  - **AC:** reassign never leaves two ACTIVE rows; audit-logged; integration test covers the atomic swap.
  - *Backend-only (no FE changes — `ReservationDialog`'s cancel-then-create flow is untouched; this is an alternative atomic primitive). New `IUnitOfWork`/`EfUnitOfWork` (`Domain/Interfaces/IUnitOfWork.cs`, `Infrastructure/Data/EfUnitOfWork.cs`) wraps `Database.BeginTransactionAsync`/`CommitAsync` (+ `CreateExecutionStrategy`) around two ordered `SaveChangesAsync` calls — cancel-old-then-save, create-new-then-save — inside one transaction, so the partial unique index never sees two ACTIVE rows and no other connection ever observes zero ACTIVE rows (proven, not just asserted: see below). New `Reservation.CreateForReassignment` registers a single `ReservationReassignedEvent` (old id, new id, space, lot, old driver, new driver, actor) → new `ReservationReassignedEventHandler` writes one combined audit entry (`reservation.reassigned`); the old row's existing `Cancel()` still fires its own `reservation.cancelled` entry too. Docker was available in this environment — `dotnet build` (0 warnings/errors, NsDepCop clean), unit tests (`ReservationTests`, `ReassignReservationHandlerTests`, 36/36 passing), and Testcontainers-backed integration tests actually executed against real Postgres (`ReservationReassignTests`, 3/3 passing, plus the 2 pre-existing `ReservationConcurrencyTests` still green): one test proves the naive wrong statement order really does throw a unique-violation (the trap is real), one proves the actual `EfUnitOfWork` ordering always leaves exactly one ACTIVE row, and one proves a second, independent connection never sees zero ACTIVE rows for the space while the swap's transaction is still open. Did not get to a live `dotnet run --project src/Parkin.AspireHost` + browser smoke test.*

---

## Phase 5 — ★ Access Events, Decision Engine & Occupancy (Epic E) — highest value & risk

- [x] **T5.1 — E3 EntryDecisionService + OccupancyCalculator (pure, tested first).** *Deps: none beyond domain enums.* Pure domain services in `Domain/Services/`, no I/O. Implement exact precedence: (1) unknown+RESTRICTED→DENY `NOT_AUTHORIZED`; (2) RESTRICTED+no grant/reservation→DENY `NOT_AUTHORIZED`; (3) reserved holder→ALLOW `RESERVED` **bypassing full**; (4) general+full+BLOCK→DENY `LOT_FULL`; (5) else ALLOW `GENERAL`. `OccupancyCalculator`: `general free = max(0, capacity − active GENERAL sessions)`, overflow flag, reserved bypass. **Exhaustive unit tests — this is the core IP, cover well above the ~30% target.**
  ...
  - *`EntryDecisionContext`/`OccupancyContext` are sealed records built only via a validating `Create(...)` factory (private `init` setters) so an invalid state — e.g. an unknown plate carrying a grant/reservation, or a reservation without a space label — can't be constructed at all, not just rejected by the decision logic. `EntryDecisionService`/`OccupancyCalculator` are plain classes implementing `IEntryDecisionService`/`IOccupancyCalculator` (no constructor deps), ready for DI injection in T5.2. Rule interpretations pinned by named tests, for T5.2's author: an unknown plate on a full **OPEN** lot is `LOT_FULL`, never `NOT_AUTHORIZED` (rule 1 only fires under RESTRICTED); a **grant does not bypass full/overflow** — it only clears the RESTRICTED gate, then the holder is an ordinary general entrant (only an active **reservation** bypasses full, per rule 3); `HasActiveGrant`/`HasActiveReservation` are already-filtered "currently active" booleans — an expired/revoked grant or a cancelled reservation both collapse to `false`, no separate state. 84/84 unit tests passing (57 new for this task); `dotnet test --collect:"XPlat Code Coverage"` shows 100% line/branch coverage on every class under `Domain/Services/`.*
- [x] **T5.2 — E1/E2 Ingestion endpoint + session lifecycle.** *Deps: T5.1, T1.4, T3.2, T4.1.* `POST /api/v1/access-events` (API-key auth, required `Idempotency-Key`, always **200 + decision body**). `IngestAccessEventCommandHandler` in one transaction: idempotency check (`ux_access_event_idempotency`, replay returns original, no double-count) → `SELECT lot FOR UPDATE` → resolve plate→driver/grant/reservation/active-GENERAL-count → `Decide()` → ENTER+ALLOW opens session (pool, space_id only for reserved) / EXIT closes most-recent open session else records anomaly, **never negative** → persist immutable `AccessEvent` → audit. Index `ix_session_active_lot_pool`. *(Backend only — no FE deliverable; the gate is machine-facing and occupancy UI lands in T5.3. Contract decisions: an unmatched EXIT is `DENY NO_OPEN_SESSION` (the barrier config must still let the car out); unknown lot → `200 DENY LOT_NOT_FOUND`, unpersisted since `access_events.lot_id` is a non-null FK, audit-logged only; archived lot → `200 DENY LOT_ARCHIVED`, persisted. Only `401` (bad key) and `400` (malformed body / missing `Idempotency-Key`) escape the 200 contract. Enum wire values are PascalCase (`NotAuthorized`, not `NOT_AUTHORIZED`) to match every other endpoint and the persisted column values. `ILotRowLocker` + `IEntryDecisionService`/`IOccupancyCalculator` registered in DI (T5.1 left the latter two unregistered). Testcontainers bumped 4.3.0 → 4.14.0 — the old version's transitive SSH.NET tripped NuGet audit under `TreatWarningsAsErrors`, blocking restore of both test projects. All 129 tests green: 13 new units, 5 integration (idempotent replay, unique-index race, per-lot `FOR UPDATE` admitting exactly one at the last free space, exit anomaly, enter→exit→re-enter), 7 functional over the real HTTP pipeline.)*
...
- [x] **T7.1 — G1 Audit query.** *Deps: T1.2 (writing has run since Phase 1 via the SaveChanges interceptor + explicit decision logging).* `GET /audit?from&to&actor&actor_type&entity` (`SystemAdmin`, read-only; route has no `/api/v1` prefix — none of this repo's other endpoints use one either). Enforce append-only (revoke UPDATE/DELETE on the table for the app DB role). Indexes `ix_audit_occurred_at/actor/entity` already existed from the Phase-1 `AddAuditLogEntries` migration, so no new index migration was needed. FE: `features/audit/` (AuditFilters, AuditTable) under `settings/audit`.
  - **AC:** log filterable by date/actor/actor-type/entity; not editable/deletable through the app; access decisions + admin mutations all appear. *(No `EntryDecisionService`/access-events yet — Phase 5 is still open — so only admin mutations appear in data today; the query itself is decision-agnostic and will surface access-decision entries once Phase 5 lands, since they'll write through the same `AuditLogEntry` table.)*
  - **Append-only caveat:** the `AuditLogAppendOnly` migration issues `REVOKE UPDATE, DELETE ON TABLE "AuditLogEntries" FROM <current_user>` (dynamic, so it always targets whichever role is running migrations/queries). Verified for real: table ownership does **not** bypass an explicit `REVOKE` for UPDATE/DELETE in Postgres (confirmed empirically against a throwaway non-superuser role that owned the table) — SELECT/INSERT kept working, UPDATE/DELETE returned `permission denied for table`. In local/dev the app connects as the Postgres superuser (`Username=postgres` in `appsettings.json`), and superusers bypass all GRANT/REVOKE checks by design, so the migration has **no observable effect locally** — this is real defense-in-depth for a properly scoped non-superuser production role, not a no-op by design flaw.
